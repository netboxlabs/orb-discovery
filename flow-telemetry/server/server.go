...
// Server represents the flow-telemetry HTTP server.
type Server struct {
	router     *gin.Engine
...
// NewServer returns a new flow-telemetry server.
...
// Start starts the HTTP server in a goroutine and returns an error channel.
func (s *Server) Start() <-chan error {
	errCh := make(chan error, 1)
	go func() {
		s.logger.Info("starting flow-telemetry server", "address", s.httpServer.Addr)
...
// Stop gracefully shuts down the HTTP server and all policies.
func (s *Server) Stop() {
