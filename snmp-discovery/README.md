...
Opt-in population of the NetBox `asset_tag` field on discovered devices, sourced from the standard ENTITY-MIB `entPhysicalAssetID` column.

Controlled by the `discover_asset_tags` option (default `false`):

| Value | Behaviour |
|---|---|
| `false` *(default)* | `entPhysicalAssetID` is not walked; `asset_tag` is only set when supplied via `defaults.asset_tag`. |
| `true` | `entPhysicalAssetID` is walked and applied to the master device (standalone) or per-member device (stack). |

Tag application rules:
- The `defaults.asset_tag` value always takes precedence; a wire tag is only applied when `asset_tag` is not already set via defaults.
- On virtual-chassis targets, each stack member receives its own per-row tag. `defaults.asset_tag` is never copied to individual members — NetBox `asset_tag` is unique, so one operator-supplied tag applied to N members would collapse them onto one NetBox row.
- Tags that are empty, contain invalid characters, or match well-known placeholder values are silently suppressed.
- Duplicate tags across chassis rows within the same target are warn-logged and suppressed to avoid NetBox uniqueness violations.
- A tag already seen on another target of the same policy is suppressed with a warning (first target wins). This guards against vendor-cloned EEPROM values causing two different devices to merge onto one NetBox record via Diode's highest-precedence asset-tag matcher.
...
Opt-in discovery of VRF instances defined on the target device, sourced from the MPLS-L3VPN-STD-MIB and Cisco-VRF-MIB tables.

Controlled by the `discover_vrfs` option (default `false`):

| Value | Behaviour |
|---|---|
| `false` *(default)* | VRF MIB tables are not walked; IP addresses are assigned using the `defaults.vrf`, `defaults.vrf_ipv4`, and `defaults.vrf_ipv6` values when set. |
| `true` | VRF MIB tables are walked; discovered VRFs are attached to IP addresses by interface membership. Per-address-family defaults (`vrf_ipv4`, `vrf_ipv6`) remain the fallback for interfaces not covered by a walked VRF entry. |
