...
class OrbPackageFinder(importlib.abc.MetaPathFinder):
    """
    sys.meta_path finder that resolves modules from orb-agent bundle directories.
    ...
    def find_spec(self, fullname: str, path, target=None):
        ...
        for bundle_dir in self._active_bundle_dirs():
            ...
            if (candidate / "__init__.py").is_file():
                ...
                if spec is not None:
                    logger.debug(f"OrbPackageFinder: resolved '{fullname}' from {bundle_dir}")
                    return spec
...
def _maybe_evict(package_name: str) -> None:
    ...
    by orb-agent's PackageManager takes effect without restarting the worker.
    ...
    Args:
    ----
        package_name: Top-level module name (e.g. "nbl_cisco_meraki").

    """
    # Derive the bundle directory name: module names use underscores,
    # bundle dirs may use hyphens (e.g. nbl-cisco-meraki). Check both.
    candidates = [
    ...
    if current is None:
        # Not a bundle-managed package; nothing to do.
        return
    ...
    if cached != resolved:
        ...
        logger.debug(f"OrbPackageFinder: '{package_name}' is current, no eviction needed")
