...
func TestConvenienceFunctions(t *testing.T) {
	...
	t.Run("GetDiscoveryAttempts", func(t *testing.T) {
		counter := metrics.GetDiscoveryAttempts()
		assert.NotNil(t, counter)

		// Test that it's the same as getting the counter directly
		directCounter := metrics.GetCounter("discovery_attempts", "Number of SNMP discovery attempts")
		assert.Same(t, counter, directCounter)
	})
}
