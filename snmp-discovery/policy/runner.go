...
// mapping.TranslateAsStack. First target to discover a tag owns it for
// the lifetime of this runner (one policy); the same target may
// re-claim its tags on every cycle. A different target reporting an
// already-owned tag — vendor-cloned EEPROM values, mislabeled gear —
// is suppressed with a warn, because emitting it would make Diode's
// highest-precedence matcher merge two devices onto one NetBox record.
//
...
func (r *Runner) assetTagClaimer(targetHost string) func(string) bool {
	return func(tag string) bool {
		r.assetTagOwnersMu.Lock()
		defer r.assetTagOwnersMu.Unlock()
		owner, ok := r.assetTagOwners[tag]
		if !ok {
			r.assetTagOwners[tag] = targetHost
			return true
		}
		if owner == targetHost {
			return true
		...
		r.logger.Warn("asset tag skipped: already discovered on another target of this policy",
			"asset_tag", tag, "target", targetHost, "owner_target", owner)
		return false
	}
}
...
func (r *Runner) queryTarget(ctx context.Context, target config.Target) ([]diode.Entity, error) {
	...
	entitiesForTarget = mapping.TranslateAsStack(entitiesForTarget, oids, ifIndexByIface, r.assetTagClaimer(targetHost), r.logger)
