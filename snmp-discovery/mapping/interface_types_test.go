...
var tests = []struct {
	...
	},
	{
		...
		defaultInterfaceType: "",
		expectedNetboxType:   "other-wireless",
		description:          "propDocsWirelessDownstream should map to other-wireless",
	},
	{
...
func TestInterfaceTypeMap_Completeness(t *testing.T) {
	...
	testedIfTypes := make(map[int]bool)
	...
	for _, ifType := range ifTypes {
		assert.True(t, testedIfTypes[ifType], "Missing test for ifType %d", ifType)
	}
}
