...
// Server represents the probe-telemetry server
type Server struct {
	router     *gin.Engine
...
// NewServer returns a new probe-telemetry server
...
// Start starts the probe-telemetry server
func (s *Server) Start() <-chan error {
	errCh := make(chan error, 1)
	go func() {
		s.logger.Info("starting probe-telemetry server", "address", s.httpServer.Addr)
...
// Stop stops the probe-telemetry server
func (s *Server) Stop() {
